...
uint8_t Audio_Buffer_Samples[AUDIO_BUFFER_SIZE*8]__attribute__((at(0x20000BFC)));
...
extern FIL FileRead_Copy_DestM, FileRead_Copy_Dest, FileRead_Sx;
...
//                    WaveDataLength = waveformat.SubChunk2Size;
                  ...
                  //  printf("Sample buffer size: %d bytes\r\n", samples_buffer_size);
                    ...
                    bytesread = bytesread + 2; // to start with left channel
...
uint8_t delete_music_int(void){
        ...
        FRESULT fr;  
        /*
        fr = f_close(&FileRead_S);
     //   fr = f_close(&FileRead_D);
        fr = f_open(&FileRead_S, "0:/Audio.wav" , FA_WRITE);
        */
...
FRESULT check_player_stat(uint8_t tmp_stats){
        ...
        if(tmp_stats != 2){
                        ...
                        rcx = f_open(&FileRead_S, path , FA_READ);
                        if(rcx != FR_OK){
                                return rcx;
                        }
        }
        
        rcx = f_read(&FileRead_S, tmp_buffer_tmp, sizeof tmp_buffer_tmp, &brc); /* Read a chunk of data from the source file */
...
uint8_t ReadWirKey(void){
       ...
       char net_id_tmp[3] = {0};
       char net_key_tmp[5] = {0};
       char net_ch_tmp[3] = {0};
       char net_ch_crc[3] = {0};
    
       
        uint8_t netid_tmp = 0;
        uint16_t netkey_tmp = 0;
        uint8_t netch_tmp = 0;
        uint8_t netch_crc = 0;
       
       rc = f_open(&fileObj, FILENAME_KEY, FA_READ);
       if (rc) {
             return 200;// no key file
       }
       else {
            
                  /* Read a chunk of file */
                  rc = f_read(&fileObj, F_buffer, sizeof F_buffer, &br);
                   /*
                   if (rc || !br) {
               ...
               if(F_buffer[0] == '#' && F_buffer[1] == '#' && F_buffer[12] == '$' && F_buffer[13] == '$'){//validate first and end chars
                       

                       net_id_tmp[0] = F_buffer[2];
                       net_id_tmp[1] = F_buffer[3];
                       net_id_tmp[2] = 0;
                       
                       
                       netid_tmp = (uint8_t)strtol(net_id_tmp, 0, 16);
                       if(netid_tmp == 0) return 0;
                       
                       
                       net_key_tmp[0] = F_buffer[4];
                       net_key_tmp[1] = F_buffer[5];
                       net_key_tmp[2] = F_buffer[6];
                       net_key_tmp[3] = F_buffer[7];
                       net_key_tmp[4] = 0;
                       
                       
                       Network_Name[0] = F_buffer[4];
                       Network_Name[1] = F_buffer[5];
                       Network_Name[2] = F_buffer[6];
                       Network_Name[3] = F_buffer[7];
                       
                       
                       netkey_tmp = (uint16_t)strtol(net_key_tmp, 0, 16);
                       if(netkey_tmp == 0) return 0;
                       
                       net_ch_tmp[0] = F_buffer[8];
                       net_ch_tmp[1] = F_buffer[9];
                       net_ch_tmp[2] = 0;
                       
                       netch_tmp = (int)strtol(net_ch_tmp, 0, 16);
                       if(netch_tmp == 0) return 0;
                       if(netch_tmp > 80) return 0;
                       
                       

                       net_ch_crc[0] = F_buffer[10];
                       net_ch_crc[1] = F_buffer[11];
                       net_ch_crc[2] = 0;
                       
                       netch_crc = (uint8_t)strtol(net_ch_crc, 0, 16);
                       
                       uint8_t key_low_tmp = 0xFF & (netkey_tmp >> 8);
                       uint8_t key_high_tmp = 0xFF & (netkey_tmp >> 0);
                       uint8_t tmp_crc_wireless = netch_tmp ^ netid_tmp ^ key_low_tmp  ^ key_high_tmp;

                       if(tmp_crc_wireless == netch_crc || (net_ch_crc[0] == 'X' && net_ch_crc[1] == 'X')){
                                save_433_config_param(netch_tmp, netid_tmp, netkey_tmp, 1);//save settings to modules memory 
                                return 100;
                       }else{
                                return 1; //error key not valid
                       }
               }else{
                       return 2; //error key not valid
               }
       }
